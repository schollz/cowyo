...
func TestBrowserRootRendersLandingPage(t *testing.T) {
	...
	for description, marker := range map[string]string{
		...
		"private key note":        `start private so the server receives`,
		"private ciphertext note": `only ciphertext.`,
...
func TestBrowserAboutRendersDedicatedPage(t *testing.T) {
	...
	for description, marker := range map[string]string{
		...
		"private how note":     `Starting private means the server never receives plaintext.`,
